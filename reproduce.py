...
def prepare():
 ...
 with tarfile.open(fileobj=Parts(files),mode='r|gz') as t:
  for m in t:
...
def calculations():run(['Rscript','--vanilla','-e',"source('scripts/p15/activate_p15_environment.R'); targets::tar_make(script='../tools/replication_targets.R')"],cwd=WORK)
