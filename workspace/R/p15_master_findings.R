...
p15_master_note_alerts <- function(registry,review,results) {
  ...
  # An edit after review invalidates that note review even if data are unchanged.
  if ("reviewed_note_sha256" %in% names(z)) {
    z[,current_note_sha256:=vapply(note,function(p)if(file.exists(p))
      digest::digest(file=p,algo="sha256") else NA_character_,character(1))]
    z[,review_required:=review_required | (status=="live" &
      (is.na(reviewed_note_sha256)|is.na(current_note_sha256)|reviewed_note_sha256!=current_note_sha256))]
  }
  z
}
...
p15_master_report <- function(config,raw,done) {
  ...
  lines<-c("# Master refresh report","",paste0("Configuration: ",config$version),
    ...
    if(!is.null(config$revised_peer_config)) "- The revised private working peer method is owner-adopted under PEER-REFERENCE-20260912-V1. This build does not authorize a public/canonical release." else "- No finding, method or canonical dataset is automatically promoted; interpretation requires explicit review and methodological decisions retain their owner-approval requirements.",
  ...
  bundle<-list(build_id=basename(out),schema_id="SCHEMA-P15-MASTER-REFRESH-V1",
    ...
    selection_id=if(!is.null(config$revised_peer_config))"SEL-P15-PRIVATE-RULES1TO5-PISR-RADIUS3-V1" else "SEL-P15-REFERENCE-REGION-CORRECTED-V1",
    source_package_ids=paste0("SRC-P15-RAW-CLOSURE-20260906;SRC-P15-SOURCE-CLOSURE-20260907;SRC-WB-COUNTRIES-LOCAL-PEER-20260909",if(!is.null(config$revised_peer_config))";SRC-P15-REVISED-PEER-INPUTS-20260912-V1" else ""))
  ...
  archived<-vapply(context,function(p) {
    if(!grepl("^/",p) && grepl("[.][.]/",p))stop("Invalid context path: ",p)
    relative<-if(grepl("^/",p))file.path("absolute",paste0(substr(digest::digest(p),1,12),"_",basename(p))) else p
    dest<-file.path(out,"context",relative);dir.create(dirname(dest),recursive=TRUE,showWarnings=FALSE)
  ...
  jsonlite::write_json(c(bundle,list(release_state="private_working_candidate_not_canonical",
    analysis_source_manifest=config$analysis_snapshot_manifest,
    stage_estimators=if(!is.null(config$revised_peer_config))"accepted_revised_peers_other_formulas_preserved" else "inherited_without_method_change")),
