...
p15_master_external <- function(config,id) {
  ...
  paths<-p15_master_manifest_paths(file.path(ref,"input_manifest.csv"))
...
p15_master_code <- function(config,id,script) {
  ...
  sort(unique(c(code,script,"R/p15_master.R","renv.lock","scripts/p15/activate_p15_environment.R")))
}
p15_master_raw_files <- function(spec,snapshot_manifest) {
  ...
  downstream_only<-"R/p15_thesis_benchmark_inference.R"
