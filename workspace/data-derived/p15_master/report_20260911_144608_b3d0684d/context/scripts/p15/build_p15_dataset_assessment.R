...
# Audit actual accepted peer membership. These are existing estimates, not a refit.
seed<-p[,.(analysis_year,peer_iso3=iso3,accepted_primary=primary,seed_id=primary_usd_source_evidence_row_id)]
mm<-merge(members,seed,by=c("analysis_year","peer_iso3"),all.x=TRUE)
peer_calc<-members[,.(member_median=median(peer_rate_pct),member_count=.N),by=.(analysis_year,iso3=target_iso3)]
pc<-merge(p,peer_calc,by=c("analysis_year","iso3"),all.x=TRUE)
member_checks<-data.table(check_id=c("no_self_peer","all_members_current_primary",
  "peer_seed_identifiers","peer_medians_reproduce","peer_counts_reproduce","peer_members_unique"),
  passed=c(all(mm$target_iso3!=mm$peer_iso3),all(is.finite(mm$accepted_primary)&abs(mm$peer_rate_pct-mm$accepted_primary)<1e-10),
    all(mm$peer_source_evidence_row_id==mm$seed_id),
    ...
    !anyDuplicated(members[,.(analysis_year,target_iso3,peer_iso3)])))
tables$peer_membership_checks<-member_checks
# A bounded input-completeness sensitivity, not a replacement for accepted peers.
# Reproduce old pools first, then fill only missing region from the saved WB file.
geo_trials <- rbindlist(lapply(seq_len(nrow(p)),function(i){
  target<-as.data.frame(p[i]);seed<-as.data.frame(p[analysis_year==target$analysis_year &
    iso3!=target$iso3 & primary_eligible==TRUE])
  oldpool<-p15_peer_similarity_pool(seed,target,3L)
  oldrate<-if(nrow(oldpool$pool)>=3)median(oldpool$pool$primary) else NA_real_
  seed$rating_source_region<-ifelse(seed$peer_region_missing,seed$wb_region,seed$rating_source_region)
  target$rating_source_region<-ifelse(target$peer_region_missing,target$wb_region,target$rating_source_region)
  newpool<-p15_peer_similarity_pool(seed,target,3L)
  newrate<-if(nrow(newpool$pool)>=3)median(newpool$pool$primary) else NA_real_
  data.table(iso3=target$iso3,analysis_year=target$analysis_year,
    historical_lmic_reporting_scope=target$historical_lmic_reporting_scope,
    selected_tier=target$selected_tier,primary=target$primary,
    accepted_peer=target$peer_rate_pct,old_rebuilt=oldrate,geography_filled_peer=newrate,
    old_pool=oldpool$rule,new_pool=newpool$rule,region_was_missing=target$peer_region_missing,
    old_peer_count=nrow(oldpool$pool),new_peer_count=nrow(newpool$pool))
}))
stopifnot(all(abs(geo_trials$accepted_peer-geo_trials$old_rebuilt)<1e-10,na.rm=TRUE),
  all(is.finite(geo_trials$old_rebuilt[is.finite(geo_trials$accepted_peer)])))
...
tables$geography_sensitivity_summary<-geo_trials[historical_lmic_reporting_scope==TRUE,
  .(n=.N,missing_region=sum(region_was_missing),changed=sum(abs(change_pp)>1e-10,na.rm=TRUE),
    ...
    old_global=sum(old_pool=="global_min3"),new_global=sum(new_pool=="global_min3")),by=selected_tier]
