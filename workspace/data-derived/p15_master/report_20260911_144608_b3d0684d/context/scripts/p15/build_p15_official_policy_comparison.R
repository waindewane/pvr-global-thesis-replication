...
args<-commandArgs(trailingOnly=TRUE);out<-if(length(args))args[1] else "data-derived/p15_official_policy_comparison_20260908_v1"
mapping<-if(length(args)>1)args[2] else Sys.getenv("P15_DAC_BASE","data-derived/p15_official_dac_mapping_20260908_v1")
if(dir.exists(out))stop("Refusing overwrite")
old<-Sys.getenv("P15_POLICY_BASE","data-derived/p15_policy_comparison_20260908_v2");base<-Sys.getenv("P15_ANALYSIS_BASE","data-derived/p15_analysis_candidate_20260907_v1")
