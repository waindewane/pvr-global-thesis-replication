...
out <- if(length(args))args[1] else "data-derived/p15_pv_annotation_followup_20260909_v1"
...
stopifnot(nrow(x)==762L,!anyDuplicated(x[keys]),!anyDuplicated(alt[c(keys,"tier")]))
...
stopifnot(nrow(rev)==19L,max(abs(rev$ge_a-rev$ge_b-rev$ge_difference_a_minus_b))<1e-8)
