...
fwrite(lmic[selected_peer_new==TRUE,.(groups=.N,donor_memberships=sum(n_peers),primary=sum(primary_members),ids=sum(ids_members),
  secondary=sum(secondary_members),rating_implied=sum(rating_implied_members),shadow_matched_members=sum(donor_shadow_members),
...
if(nrow(sc)){
 fwrite(sc[,.(input_country_years=.N,finite_shadow_grades=sum(is.finite(shadow_notch)),
   ...
   conclusion=if(any(is.finite(shadow_notch)&scorecard_complete))'Numerically usable grades for this named scenario; source adaptations remain separate' else if(any(is.finite(shadow_notch)))'Conditional shadow scenarios are built; no exact-method certification' else 'No finite shadow estimates'),
...
checks<-data.table(check=c('production_peer_778_rates','production_rule_counts','production_peer_membership','actual_moodys_grades_match_current_core',
  ...
  'rating_implied_donors_only_in_authorized_PISR_sensitivity','current_tier_eligibility_and_source_priority_reconciled','first_qualifying_rule','stronger_selected_tiers_and_rates_unchanged',
...
stopifnot(all(p[!grepl('^PISR',method),rating_implied_members]==0))
fwrite(checks,file.path(out,'checks.csv'))
fwrite(manifest(c('experiments/p15_wb_scorecard_20260912/constructed_v1/build_peer_candidate.R',
  ...
  'experiments/p15_wb_scorecard_20260912/constructed_v1/test_matching.R',
