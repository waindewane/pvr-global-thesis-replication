...
out <- if (length(args)) args[1] else file.path("data-derived",paste0("p15_panel_disbursement_",format(Sys.time(),"%Y%m%d_%H%M%S")))
...
stopifnot(nrow(x)==nrow(p), sum(x$modern_dac_comparison)==sum(p$modern_dac_comparison),
  sum(x$bullet_extension_case)==sum(p$bullet_extension_case), all(x$ordinary_scenario_with_bullet),
...
checks <- data.table(check=c("current_baselines_preserved","current_modern_rows_retained",
  "current_exact_equal_bullets_retained","terms_join_unique_and_complete","eligible_inventory_exactly_matches_baseline",
  ...
  sum(x$modern_dac_comparison)==sum(p$modern_dac_comparison),sum(x$bullet_extension_case)==sum(p$bullet_extension_case),
  ...
  nrow(z)==nrow(x)*6L*2L && !anyDuplicated(z[,.(case_id,profile_id,reference)]),
