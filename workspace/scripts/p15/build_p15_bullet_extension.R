...
out <- if(length(args))args[[1]] else file.path("data-derived",paste0("p15_bullet_",format(Sys.time(),"%Y%m%d_%H%M%S")))
...
parent <- Sys.getenv("P15_PV_FIRST_BASE", p15_current_input("pv_first"))
inventory_path <- file.path(parent,"country_year_creditor_inventory.csv")
tiers_path <- file.path(Sys.getenv("P15_ANALYSIS_BASE", p15_current_input("dataset")),"tier_eligibility.csv")
...
stopifnot(nrow(old_check)==nrow(old) && nrow(old_check)==sum(!paired$bullet_extension_case),max(abs(old_check$pv-old_check$market_pv_per_100))<1e-9)
