...
out<-if(length(args))args[1] else file.path("data-derived",paste0("p15_deeper_",format(Sys.time(),"%Y%m%d_%H%M%S")))
if(dir.exists(out))stop("Use a new diagnostic directory")
base<-Sys.getenv("P15_ANALYSIS_BASE", p15_current_input("dataset"))
prior<-Sys.getenv("P15_ASSESSMENT_BASE", p15_current_input("assessment"))
inputs<-character()
...
checks<-data.table(check=c("source_switches_match_input","opposite_is_subset","common_model_keys_complete","pool_targets_match_selected",
  "composition_identity","no_self_peer","all_ten_tail_pairs"),passed=c(nrow(bridge)==nrow(tr[switch==TRUE]) && !anyDuplicated(bridge[,.(iso3,analysis_year)]),
  sum(bridge$any_opposite)<=sum(bridge$old_source_bridge_available|bridge$new_source_bridge_available),
  nrow(model)==nrow(p[historical_lmic_reporting_scope==TRUE&is.finite(primary)&is.finite(moodys)&is.finite(peer)]),
  nrow(pool)==nrow(selpeer) && !anyDuplicated(pool[,.(analysis_year,target_iso3)]),
...
checks<-rbind(checks,data.table(check=c("peer_transitions_match_selected_pairs","common_peer_composition_identity"),
  passed=c(nrow(tab$peer_membership_turnover)==nrow(pt) &&
      !anyDuplicated(tab$peer_membership_turnover[,.(iso3,analysis_year)]),
...
fig("figure_5_peer_shared_information",ggplot(fdep,aes(analysis_year,value,color=variable))+geom_line()+geom_point()+
  scale_x_continuous(breaks=c(2012,2016,2020,2024))+labs(title="Many peer estimates reuse the same group of donor benchmarks",x=NULL,y="Number",color=NULL,
    caption="Actual selected LMIC peer targets versus distinct membership sets. Distinct sets can still share donors; sources include rating-implied estimates."))
