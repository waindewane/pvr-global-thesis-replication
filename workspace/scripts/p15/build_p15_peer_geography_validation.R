...
out<-if(length(args))args[1] else file.path("data-derived",paste0("p15_peer_geography_validation_",format(Sys.time(),"%Y%m%d_%H%M%S")))
base<-Sys.getenv("P15_ANALYSIS_BASE", p15_current_input("dataset"))
region_path<-Sys.getenv("P15_REGION_MAP",file.path(p15_current_input("regional"),"country_region_map.csv"))
...
revised<-p15_revised_peer_inputs(base)
inputs<-unique(c(inputs,revised$files))
...
pred<-rbindlist(list(p15_revised_geography_predictions(revised,FALSE),p15_revised_geography_predictions(revised,TRUE)))
meta<-as.data.table(p)[,.(analysis_year,iso3,country,reporting_region,historical_income_level,
  ...
  missing_current_moodys=is.na(moodys_rating_normalized)|!nzchar(moodys_rating_normalized))]
...
sample_rows<-function(x) {
  parts<-list(full=x,modern=x[analysis_year>=2018],early=x[analysis_year<=2021],late=x[analysis_year>=2022],
    ...
    missing_current_moodys=x[missing_current_moodys_a==TRUE],
...
design_note<-data.table(method=c("current","no_geography","region_only","worldwide_only"),
  rules=c("1;2;3;4;5","2;4","7","8"),sources="primary>IDS>secondary>existing_Moodys_implied",
  matching="actual Moody grade first; conservative shadow interval otherwise; maximum distance 3",
  status=c("candidate","diagnostic_rating_rules_without_geography","relaxed_diagnostic_only","relaxed_diagnostic_only"),
  interpretation="The September9 primary-only geography experiment is preserved; this replay compares the revised donor method. Missing Moody input does not establish genuinely unrated status.")
tables<-list(design_note=design_note,predictions=pred,validation_rows=v,coverage=coverage,all_available_scores=scores,
...
print(summaries[sample %in% c("full","modern","SSA","missing_current_moodys")]);print(change_summary);print(deploy_summary);print(checks)
