...
out <- if(length(args))args[[1]] else file.path("data-derived",paste0("p15_pv_interpretation_",format(Sys.time(),"%Y%m%d_%H%M%S")))
...
base <- Sys.getenv("P15_BULLET_BASE", p15_current_input("bullet"))
paths <- c(file.path(base,c("paired_pv_for_inclusion_check.csv","inventory_with_bullet_flags.csv")),
  ...
  file.path(Sys.getenv("P15_ANALYSIS_BASE", p15_current_input("dataset")),"tier_eligibility.csv"))
...
stopifnot(nrow(x)==sum(fread(paths[1])$modern_dac_comparison),all(!is.na(x$region)),!anyDuplicated(x[c("iso3","analysis_year","creditor")]))
...
checks <- tibble(check=c("modern_rows_match_current_parent","unique_case_keys","regions_complete","PV_reproduces_saved_values",
