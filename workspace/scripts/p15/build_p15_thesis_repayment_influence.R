...
out <- if (length(args)) args[[1]] else file.path("data-derived",paste0("p15_repayment_influence_",format(Sys.time(),"%Y%m%d_%H%M%S")))
...
stopifnot(nrow(x) > 0L, nrow(pairs) > 0L, all(x$analysis_year %in% 2018:2024), !anyDuplicated(x[, ..keys]))
...
stopifnot(baseline$n_reversals == nrow(saved_reversals),
...
checks <- data.table(check = c("all_consumed_table_hashes_match", "current_parent_scenarios_pairs_and_reversals",
  ...
  "no_purpose_held_secondary_in_modern_scenarios", "pakistan_no_reversal_unchanged_nonpeer_check"),
