...
out <- Sys.getenv("P15_PV_CHECK_OUTPUT",file.path("data-derived",paste0("p15_pv_three_block_",format(Sys.time(),"%Y%m%d_%H%M%S"))))
...
check("independent_all_current_PV",max(abs(pv-x$market_pv_per_100))<1e-8)
check("unique_modern_cases",nrow(x)>0L && all(x$analysis_year %in% 2018:2024)&&!anyDuplicated(x[c("iso3","analysis_year","creditor")]))
